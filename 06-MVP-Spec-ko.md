...
사용자: "비트코인 처음 사보려는데 어떻게 해?"
        |
        v
AI 추론: experience_level = beginner
        |
        v
월렛 버핏: "자네, 처음 시작하는구먼! 
          먼저 기초부터 알려주지..."
...
|   ──────────────────              ───────────────                  |
|                                                                    |
|   - 채팅 내 압축                  - 최근 20개 메시지만 유지       |
|   - 세션 종료 시 요약             - 그 이전은 버림                |
|   - 유저 레벨 기억 축적           - 추론된 프로필만 저장          |
|   - Claude Haiku 요약 호출        - 추가 AI 호출 없음             |
|                                                                    |
|   비용: 세션당 $0.001             비용: $0                        |
|   복잡도: 높음                    복잡도: 낮음                    |
|                                                                    |
...
export function FearGreedGauge({ value, label }: FearGreedGaugeProps) {
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-6">
      <h2 className="text-lg text-gray-400 mb-4">Fear & Greed Index</h2>
      
      <div className="flex items-center gap-4">
        <div className="text-5xl font-bold">{value}</div>
        <div className={`px-3 py-1 rounded-full text-sm ${getColor(value)}`}>
          {label}
        </div>
      </div>
...
export function SocialProof({ activeUsers, testimonial }: SocialProofProps) {
  return (
    <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-gray-300">
          지금 <span className="text-white font-bold">{activeUsers.toLocaleString()}명</span>이 
          월렛 버핏과 대화 중
        </span>
      </div>
...
export function useAIComment(fearGreedValue: number, priceChange: number) {
  return useQuery<AICommentData>({
    queryKey: ['ai-comment', fearGreedValue, Math.round(priceChange)],
    queryFn: async () => {
      // Sage.ai API에서 캐시된 AI 코멘트 가져오기
      const res = await fetch(
        `https://api.sage-ai.com/v1/viral/comment?fg=${fearGreedValue}&change=${Math.round(priceChange)}`
      );
      return res.json();
    },
    refetchInterval: 5 * 60_000, // 5분마다 갱신
    staleTime: 60_000,
    enabled: fearGreedValue > 0, // Fear&Greed 데이터 있을 때만
  });
}
```

#### 10.1.4 Sage.ai API (AI 코멘트 제공)

```typescript
// 메인 앱 API: app/api/viral/comment/route.ts
...
export async function GET(req: NextRequest) {
  ...
  const fg = parseInt(searchParams.get('fg') || '50');
  const change = parseInt(searchParams.get('change') || '0');
  
  // 캐시 키 (Fear&Greed 구간 + 가격 변동 방향)
  const cacheKey = `viral:comment:${Math.floor(fg / 10)}:${change > 0 ? 'up' : change < 0 ? 'down' : 'flat'}`;
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 200,
    system: `당신은 월렛 버핏입니다. 암호화폐 시장 상황에 대해 2-3문장으로 짧게 코멘트하세요.
             말투: "자네", "~일세", "~하게"
             현재 Fear & Greed: ${fg}, 24시간 변동: ${change}%`,
    messages: [{ 
      role: 'user', 
      content: '지금 시장 상황에 대해 한마디 해주세요.' 
    }],
  });
  
  const comment = response.content[0].type === 'text' 
...
function MainContent() {
  ...
  const { data: aiData, isLoading: aiLoading } = useAIComment(
    fgData?.value || 50,
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <SocialProof
          activeUsers={1247}
          testimonial={{
            text: "분할 매수 조언 덕분에 -5% 손실을 +12%로 바꿨어요",
            author: "사용자 김**",
            date: "3일 전"
          }}
        />
...
<head>
  ...
  <!-- SEO -->
  <title>Why Is Bitcoin Falling? - 실시간 BTC 분석</title>
  <meta name="description" content="비트코인이 왜 떨어졌을까? AI 투자 멘토 월렛 버핏이 실시간으로 분석해드립니다." />
  <meta name="keywords" content="비트코인, BTC, 암호화폐, 하락, 분석, AI, 투자" />
  
  <!-- Open Graph -->
  <meta property="og:type" content="website" />
  <meta property="og:title" content="Why Is Bitcoin Falling?" />
  <meta property="og:description" content="AI 투자 멘토가 분석하는 실시간 비트코인 시장" />
  <meta property="og:image" content="https://whybitcoinfallen.com/og-image.png" />
  <meta property="og:url" content="https://whybitcoinfallen.com" />
  
  <!-- Twitter -->
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="Why Is Bitcoin Falling?" />
  <meta name="twitter:description" content="AI 투자 멘토가 분석하는 실시간 비트코인 시장" />
  <meta name="twitter:image" content="https://whybitcoinfallen.com/og-image.png" />
...
```yaml
# .github/workflows/deploy-viral.yml
name: Deploy WhyBitcoinFallen
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Install & Build
        working-directory: sites/why-bitcoin-fallen
        run: |
          npm ci
          npm run build
      ...
      - name: Deploy to S3
        ...
        with:
          args: --delete
        env:
          AWS_S3_BUCKET: whybitcoinfallen.com
          AWS_ACCESS_KEY_ID: ${{ secrets.AWS_ACCESS_KEY_ID }}
          AWS_SECRET_ACCESS_KEY: ${{ secrets.AWS_SECRET_ACCESS_KEY }}
          SOURCE_DIR: 'sites/why-bitcoin-fallen/dist'
          
      - name: Invalidate CloudFront
        uses: chetan/invalidate-cloudfront-action@v2
        env:
          DISTRIBUTION: ${{ secrets.CF_DISTRIBUTION_VIRAL }}
          PATHS: '/*'
          AWS_ACCESS_KEY_ID: ${{ secrets.AWS_ACCESS_KEY_ID }}
          AWS_SECRET_ACCESS_KEY: ${{ secrets.AWS_SECRET_ACCESS_KEY }}
```
...
|                                                           |
|  [기능 1]        [기능 2]        [기능 3]                 |
|  실시간 분석     섀도우 포트폴리오  능동적 알림           |
|  시장 데이터     버핏 추천 따라    급변 시 먼저           |
|  기반 조언       수익률 추적       알려드려요             |
|                                                           |
+----------------------------------------------------------+
|                                                           |
